...
impl OpDeployerConfig {
    ...
    async fn apply_contract_deployments(
        ...
        let cmd = vec![
            ...
            "-c".to_string(),
            format!(
                "cat {container_config_path_str}/intent.toml && op-deployer --cache-dir {container_config_path_str}/.cache apply --workdir {container_config_path_str} --l1-rpc-url {l1_rpc_url} --private-key {private_key} --tx-prepare-retry-interval 100ms --tx-prepare-max-attempts 100",
    ...
    pub async fn deploy_contracts(
        ...
        tracing::info!(
            ...
            "op-deployer: init (generate intent) complete"
        );
        ...
        tracing::info!(
            elapsed_ms = step_start.elapsed().as_millis() as u64,
            "op-deployer: intent updated with Anvil accounts"
        );
        ...
        tracing::info!(
            elapsed_ms = step_start.elapsed().as_millis() as u64,
            "op-deployer: apply (deploy contracts) complete"
        );
        ...
        tracing::info!(
            elapsed_ms = step_start.elapsed().as_millis() as u64,
            "op-deployer: inspect (generate genesis + rollup) complete"
        );
