...
/// Container port configuration for op-proposer.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct OpProposerContainerPorts {
    pub rpc: u16,
    pub metrics: u16,
}

impl Default for OpProposerContainerPorts {
    fn default() -> Self {
        Self {
...
/// Bound host port configuration for op-proposer.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct OpProposerBoundPorts {
    pub rpc: Option<u16>,
    pub metrics: Option<u16>,
}

impl Default for OpProposerBoundPorts {
    fn default() -> Self {
        Self {
            rpc: Some(0),
            metrics: None,
...
/// Unified port configuration for op-proposer.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(tag = "mode", rename_all = "lowercase")]
pub enum OpProposerPorts {
    Host { bound_ports: OpProposerBoundPorts },
    Bridge {
        container_name: String,
        container_ports: OpProposerContainerPorts,
        bound_ports: OpProposerBoundPorts,
    },
}

impl OpProposerPorts {
...
impl OpProposerBuilder {
    ...
    pub async fn start(
        ...
        let (container_ports, bound_ports) = match &self.ports {
            OpProposerPorts::Host { bound_ports } => (OpProposerContainerPorts::default(), bound_ports.clone()),
            OpProposerPorts::Bridge { container_ports, bound_ports, .. } => (*container_ports, bound_ports.clone()),
        };
        ...
        let actual_bound_ports = OpProposerBoundPorts {
        ...
        let runtime_ports = match &service_handler.ports {
            ContainerPorts::Host { .. } => OpProposerPorts::Host {
                bound_ports: actual_bound_ports,
            },
            ContainerPorts::Bridge { container_name, .. } => OpProposerPorts::Bridge {
